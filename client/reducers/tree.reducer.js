...
export default function(state=INITIAL_STATE, action) {
    let tree = JSON.parse(JSON.stringify(state));
    let root = JSON.parse(JSON.stringify(state.cards));
    let card = action.payload;
    let activeCard = getCard(tree.activeCard, tree.cards);
    /* tree.debugging = JSON.stringify(action.payload);*/

    switch(action.type) {
	case 'CREATE_CARD':
	    /* Creating a card relative to itself */
        let relativeTo = getCard(state.activeCard, root);
	    ...
	    return tree;
	case 'DROP_CARD':
        let droppedCard = action.payload.card;
        relativeTo = getCard(action.payload.relativeTo, root);
        direction = action.payload.direction;

        /* Delete card */
        let updatedRoot = deleteCard(droppedCard, root);
	...
	case 'DELETE_CARD':
        ...
        let toDelete = activeCard;
        if (action.payload) {
		toDelete = action.payload;
	    }
        let newActiveCard = getCardRelativeTo(toDelete, root, "up");
        ...
        updatedRoot = deleteCard(toDelete, root);
        tree.cards = updatedRoot;
	...
	case 'CHECKBOX':
        ...
        card = getCard(cardId, root);
        let content = card.content;

        /* Find [ ] or [X] */
        let checkboxRegexp = new RegExp(/\[(X| )\]/, 'ig');
        let i = 0;
